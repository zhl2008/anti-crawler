...
function MakeOneTag(&$dtp, &$refObj, $parfield='Y')
{
    ...
    {
        // var_dump($ctag->CAttribute->Items["name"]);
        // exit();
        file_put_contents('/tmp/1.txt', $ctag->CAttribute->Items["name"] . "\n", FILE_APPEND);
        ...
        if(in_array($tagname,$alltags))
        {
            ...
            // if($ctag->CAttribute->Items["name"]=='700x90'){
                // var_dump($tagid);
                // $dtp->Assign(20,$funcname($ctag,$refObj));
                // var_dump($funcname);
                // var_dump($ctag);
                // var_dump($refObj);
                // exit('haozi');
